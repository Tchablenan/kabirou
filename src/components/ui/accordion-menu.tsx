    ...
    <AccordionPrimitive.Item className="flex" {...props}>
      <AccordionPrimitive.Header className="flex w-full">
        <AccordionPrimitive.Trigger
          ...
          onClick={(e) => {
            if (onItemClick) {
              onItemClick(props.value as string, e as any);
            }

            if (onClick) {
              onClick(e as any);
            }
            // Temporarily removing e.preventDefault() as it breaks Next.js Link navigation when asChild is used.
          }}
          onKeyDown={(e) => {
            if (e.key === 'Enter') {
              // Intentionally left e.preventDefault() here for keyboard navigation if needed,
              // but actually Next.js Link handles Enter natively if it's an anchor.
