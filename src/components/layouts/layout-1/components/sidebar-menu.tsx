...
export function SidebarMenu() {
  ...
  const getLocalizedPath = (path: string | undefined) => {
  ...
  const matchPath = useCallback(
    (path: string): boolean => {
      const locPath = getLocalizedPath(path);
      return locPath === pathname || (locPath.length > 1 && pathname.startsWith(locPath) && locPath !== '/layout-1');
    },
    [pathname],
  );
  ...
  const buildMenuItemRoot = (item: MenuItem, index: number): JSX.Element => {
    if (item.children) {
      return (
        <AccordionMenuSub key={index} value={getLocalizedPath(item.path) || `root-${index}`}>
          ...
          <AccordionMenuSubContent
            type="single"
            collapsible
            parentValue={getLocalizedPath(item.path) || `root-${index}`}
            className="ps-6"
          >
            <AccordionMenuGroup>
      ...
      return (
        <AccordionMenuItem
          key={index}
          value={getLocalizedPath(item.path) || ''}
  ...
  const buildMenuItemChild = (
    ...
    if (item.children) {
      return (
        <AccordionMenuSub
          key={index}
          value={getLocalizedPath(item.path) || `child-${level}-${index}`}
        >
          ...
          <AccordionMenuSubContent
            type="single"
            collapsible
            parentValue={getLocalizedPath(item.path) || `child-${level}-${index}`}
            className={cn(
              'ps-4',
      ...
      return (
        <AccordionMenuItem
          ...
          asChild
        >
          <Link href={getLocalizedPath(item.path)}>{item.title}</Link>
        </AccordionMenuItem>
      );
    }
  };
